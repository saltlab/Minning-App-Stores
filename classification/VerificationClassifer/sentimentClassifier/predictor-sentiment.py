...
def _getCollectionIOS():
    ...
    for post in  collection_ios.find(limit=1000).batch_size(30):
            ...
            positive=(float(And_numberPosReviews)/numberOfReviews)*100

            #print "negative Reviews:"+str((float(And_numberNegReviews)/numberOfReviews)*100)+"%"
            negative=(float(And_numberNegReviews)/numberOfReviews)*100
            #print "neutral Reviews:"+str((float(And_numberNeutReviews)/numberOfReviews)*100)+"%"
            neautral=(float(And_numberNeutReviews)/numberOfReviews)*100
            iosPositiveClassifierStats={'positive':positive,'negative':negative,'neautral':neautral}
            collection_ios.find_one_and_update({"android_app_id":android_app_id}, {'$set': {'iosSentimentClassifierStats':iosPositiveClassifierStats}  })
            ...
            #print "number of apps:"+str(numberApps)
...
def _getCollectionAndroid():
    ...
    for post in  collection_ios.find(limit=1000).batch_size(30):
            ...
            neautral=(float(And_numberNeutReviews)/numberOfReviews)*100
            iosPositiveClassifierStats={'positive':positive,'negative':negative,'neautral':neautral}
            collection_ios.find_one_and_update({"android_app_id":android_app_id}, {'$set': {'androidSentimentClassifierStats':iosPositiveClassifierStats}  })
